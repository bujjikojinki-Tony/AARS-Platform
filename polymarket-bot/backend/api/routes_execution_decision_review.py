...
from backend.execution_decision_review.execution_decision_review_service import (
...
def create_execution_decision_review_router(repository) -> APIRouter:
    router = APIRouter(prefix="/api/execution-decision-review", tags=["execution-decision-review"])
    service = ExecutionDecisionReviewService(repository)

    @router.get("/summary")
    def get_execution_decision_review_summary():
        return {
            "status": "ok",
    ...
    @router.get("/reviews")
    def list_execution_decision_reviews(
        limit: int = 100,
    ...
    def get_execution_decision_review_market_bundle(market_id: str, limit: int = 100):
        return {
            "status": "ok",
            "market_id": market_id,
    ...
    @router.post("/build")
    def build_execution_decision_review(payload: dict):
        ...
        try:
            record = service.build_for_market(
                str(market_id),
                command_review_id=payload.get("command_review_id"),
                shadow_evaluation_id=payload.get("shadow_evaluation_id"),
    ...
    def build_all_execution_decision_reviews():
        try:
