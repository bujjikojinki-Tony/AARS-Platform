...
def create_deb_shadow_router(repository) -> APIRouter:
    router = APIRouter(prefix="/api/deb-shadow", tags=["deb-shadow"])
    service = DebShadowService(repository)

    @router.get("/summary")
    def get_deb_shadow_summary():
        return {
            "status": "ok",
    ...
    def get_deb_shadow_market_bundle(market_id: str, limit: int = 100):
        return {
            "status": "ok",
            "market_id": market_id,
    ...
    @router.post("/build")
    def build_deb_shadow(payload: dict):
