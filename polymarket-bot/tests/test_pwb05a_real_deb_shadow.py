...
def make_client(tmp_path) -> TestClient:
    app = create_app(
        db_path=str(tmp_path / "pwb05a.sqlite"),
        allow_network=False,
...
def test_models_serialize() -> None:
    ...
    diagnostic = DebShadowDiagnosticRecord(
        deb_shadow_diagnostic_id="dsd_test",
        deb_shadow_run_id="dsr_test",
        market_id="m1",
        sample_count=3,
...
def test_tables_created(tmp_path) -> None:
    db_path = str(tmp_path / "deb_shadow.sqlite")
    init_db(db_path)
    with sqlite3.connect(db_path) as conn:
        tables = {
            row[0]
            for row in conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('deb_shadow_runs','deb_shadow_diagnostics')"
            ).fetchall()
        }
    assert "deb_shadow_runs" in tables
    assert "deb_shadow_diagnostics" in tables
...
def test_repository_save_list_query_summary(tmp_path) -> None:
    ...
    run = DebShadowRunRecord(
        deb_shadow_run_id="dsr_repo",
        market_id="m1",
        calibration_sample_id="cs_1",
        base_probability=0.55,
        deb_probability=0.57,
    ...
    diagnostic = DebShadowDiagnosticRecord(
        deb_shadow_diagnostic_id="dsd_repo",
        deb_shadow_run_id="dsr_repo",
        market_id="m1",
        calibration_sample_id="cs_1",
        sample_count=4,
    ...
    repo.save_deb_shadow_run(run)
    repo.save_deb_shadow_diagnostic(diagnostic)

    assert len(repo.list_deb_shadow_runs()) == 1
    assert len(repo.list_deb_shadow_diagnostics()) == 1
    assert repo.get_deb_shadow_run_by_id("dsr_repo") is not None
    assert repo.get_latest_deb_shadow_run_for_market("m1") is not None
    bundle = repo.get_deb_shadow_market_bundle("m1")
    assert len(bundle.runs) == 1
    assert len(bundle.diagnostics) == 1
    summary = repo.get_deb_shadow_summary()
...
def test_service_build_for_market_from_accepted_memory(tmp_path) -> None:
    ...
    result = DebShadowService(repo).build_for_market("m1")
    run = result["run"]
    diagnostic = result["diagnostic"]

    assert run.run_status == DebShadowRunStatus.READY
    assert run.sample_count == 2
    assert run.base_probability == 0.60
    assert round(float(run.bias_adjustment or 0.0), 6) == 0.005
    assert round(float(run.deb_probability or 0.0), 6) == 0.605
    assert diagnostic.sample_count == 2
    assert any("shadow-only" in warning for warning in run.warnings)


def test_deb_shadow_apis_do_not_create_candidates_or_change_primary_engine(tmp_path) -> None:
    ...
    build = client.post("/api/deb-shadow/build", json={"market_id": "m_api"}).json()
    summary = client.get("/api/deb-shadow/summary").json()
    runs = client.get("/api/deb-shadow/runs").json()
    diagnostics = client.get("/api/deb-shadow/diagnostics").json()
    bundle = client.get("/api/deb-shadow/market/m_api").json()
...
def test_build_all_eligible_and_live_execute_rejected(tmp_path) -> None:
    ...
    build_all = client.post("/api/deb-shadow/build-all", json={}).json()
