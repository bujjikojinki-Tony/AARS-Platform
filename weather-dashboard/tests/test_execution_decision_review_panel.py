...
import weather_dashboard.ui.execution_decision_review_panel as panel


def test_build_execution_decision_review_panel_state_defaults() -> None:
    state = panel.build_execution_decision_review_panel_state()
...
def test_review_rows_map_records() -> None:
    rows = panel._review_rows(
        [
            {
                "execution_decision_review_id": "edr_1",
                "market_id": "m1",
                "decision_id": "dec_1",
                "candidate_id": "cand_1",
...
def test_bundle_summary_includes_counts() -> None:
    ...
    assert "m1" in text
    assert "2 execution decision review rows" in text
